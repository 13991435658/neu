...
const topicCtrl = {
    ...
    getTopicComment:async (req,res)=>{
        ...
        res.send({
            ok:1,
            commentInfo:result.sort((a,b)=>b.time-a.time)
        })
    },
    ...
    getTopicRank:async (req,res)=>{
        const [result] = await topicModel.getTopicRank()
        res.send({
            ok:1,
            topicRank:result.sort((a,b)=>b.hot-a.hot).slice(0,5)
        })
    }
}
